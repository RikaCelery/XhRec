...
- 时间轴缩略图是一棵**金字塔**，逐级细分到当前缩放所需的分辨率，而不是先猜一个桶宽：帧缓存
  本身的网格是 **0.1 s**，第 `L` 级的步长就是 `0.1 × 2^L`，所以每一级的帧恰好是上一级的帧加上
  中间那些——细分永远不会落空（`index × step` 必然命中缓存），粗帧也不会因为细帧还没到而失效。
  目标层级 = **比缩略图还窄的最后一级**（再细就是把画面放大），下限是单元格宽度不小于 10 px，
  因此无论录像多长、缩放多少，每屏要取的帧数都是有界的。层级序列从目标往上每隔一级取一档，
  直到"填充层级"（单元格约 128 px）——低于目标的那些档是**填充层**，在目标帧到达之前顶住画面。
- **每一帧都把单元格填满**，与轨道高度无关：图片盒就是单元格盒，用 `cover` 居中裁切，所以任何
  高度下都不会露出单元格底色（旧实现按帧自身的像素算了一个 `object-position` 偏移，把画面整个
  往下推，轨道越高露出的空带越宽）。取帧宽度按 `max(轨道高度 × 16/9, 单元格像素宽)` 动态计算，
  再向上取到 2 的幂：帧在缓存里就是 16:9 的窗口，所以 `宽度 × 9/16` 正好是能盖住轨道的高度，
  而宽度那一项保证宽格子（填充层）不会把画面横向拉大。**拖动轨道高度跨过一档时**，池里旧宽度的
  帧继续画着（不闪空），但计划会按新宽度重新取——判据是帧的宽度而不是"有没有"（`frameReady`），
  同时在途的请求作废、服务端扫描按新宽度重发；否则一次调高之后整条车道会一直糊着放大。
- 车道**只绘制这个层级序列里的帧**：内存里来自别的缩放级别的帧是数据，不是图层，把它们当填充
  会立刻把整条车道铺成一条细缝（这正是"缩略图挤在一起"那个 bug）。渲染从最细一级往下走，
  某一档只在"该时刻还没有更细的帧"处绘制；请求侧用的是更严格的判据——**只有当粗单元格覆盖的
  每个时刻都已有细帧**（缓存中、在途、或本次要请求）时，才不再请求这一档，否则画面在目标帧
  到达前会出现空洞。
- 后台按 **4 条并发**抓帧：浏览器对单个源只开约 6 条连接，留出的余量、加上服务端为悬浮预览
  单独保留的一条 ffmpeg 通道，是预览不会被后台铺满的帧卡住的真正原因。服务端另有 `frames`
  接口，客户端按层级告诉它"这段窗口按这个步长扫一遍"，服务端用自己的并发把整级帧烤好（响应
  只带网格，不带图片），客户端随后取图就是缓存命中。缓存按"离目标层级最远"淘汰。
- 悬浮窗**只在缩略图车道上**出现，并**浮在时间轴面板上方**（`position: fixed` 以车道为锚），
  所以刻度尺和轨道不会被挡住；窗口上方确实没空间时才改为贴在缩略图车道下方，且高度不超过
  车道本身。大小随车道高度变化（约 3 倍车道高，上限 240 px）。预览用的是缓存自身的 0.1 s
  网格而不是车道的粗格子；**位置锚在光标的真实坐标上**（吸附后的时刻只决定取哪一帧），否则
  深缩放时吸附误差会让悬浮窗明显偏在光标一侧；指针移动时最多只有一个请求在途，已经取过的
  时刻直接从内存出图。
...
- **退出保护**：有未落盘编辑时挂上 `beforeunload`，由浏览器弹出原生"离开网站？"提示（Chrome 只
  显示自己的文案，`returnValue` 起的是开关作用）。草稿本身按 500 ms 防抖写入，`pagehide`/标签页
  隐藏时再补一次；写失败时不会清除标记，提示继续留着并出现备份提醒。提示只在编辑尚未写入的那个
  窗口里出现，因此普通刷新不会被拦。可见的蓝色缓冲条仍响应分片完成事件，并每 250 ms 核对一次。
...
`check-timeline-preview.cjs` 使用同一套环境变量（录像需 ≥90 秒），验证时间轴预览本身：车道逐级
细分到目标层级、同一屏同时绘制多个粒度、放大后继续细分、**缩略图在任意轨道高度下都填满单元格**
（逐像素探针：同一格用两种底色各截一次图，颜色变了的像素就是没被画面盖住的地方，所以对画面内容
没有任何假设），以及调高轨道后帧会按新宽度重切、悬浮窗只在缩略图车道上出现、扫过的每个
单元格只请求一次并进入内存池、回到走过的位置立即出图、未保存编辑触发退出提示且写盘后不再提示。
