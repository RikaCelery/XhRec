...
默认开启，敏感信息在日志输出中被替换。静态模式（JWT token、cookie、认证 URL 参数、代理地址）替换为 `***`。动态字符串（主播名、用户名）在启动时注册，替换为基于 CRC32 的稳定哈希值——同一会话内保持不变，重启后更新，便于日志关联分析同时保护隐私。房间 id 与该房间的主播名**共用同一个哈希值**，所以 `roomId=` 和它对应的名字在日志里是同一个标识；id 只在 `roomId=...` 这种模式里替换，不会按裸数字做子串替换。
